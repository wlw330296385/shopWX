...
class Upload {
    ...
    public function upload($files='') {
        ...
        // 判断是否上传成功
        if (!empty($info)) {
            $uploadKey = array_keys($_FILES);
            //惠路由本地上传路径
            if (empty($_FILES[$uploadKey[0]])) {
                return;
            }
            $path  =  'Uploads/'.$info[$uploadKey[0]]['savepath'].$info[$uploadKey[0]]['savename'];
            $upRes = oss_upload($path);

            unlink($path);//删除掉服务器本地的临时文件
            
            if (!$upRes) {
                return false;
            }
            return $ossFilePath = $upRes->{'header'}['_info']['url'];//最终文件oss的路径        
        }
        //*************************添加部分结束*************************
