...
class WxController extends Controller
{
    ...
    public function checkKeyword($key)
    {
        ...
        if($key == 'debug'){
            // $msg = array();
            // $msg['touser'] = 'orh7XtxKaxm5cTwWWmsE2lTejyMg';
            // $msg['msgtype'] = 'text';
            // $str = '您未凑足28人，订单无效，请分享给好友才可生效哦~';
        ...
        if ($key == '推广二维码') {
            ...
            if (!$qrcode) {
                ...
                $msg = '专属二维码 生成失败，小海正在找工程师修复~~ 请耐心等待';
                self::$_wx->text($msg)->reply();
                F($vip['openid'], null);
                exit();
            }
            ...
            // $msg['msgtype'] = 'text';
            // $str = '长按复制下方文字，分享二维码图片';
            // $msg['text'] = array('content' => $str);
            // self::$_wx->sendCustomMessage($msg);

            // $msg = array();
            // $msg['touser'] = $vip['openid'];
            // $msg['msgtype'] = 'text';
            // $str = '【大家帮帮忙~ 长按识别二维码~ 凑28人就能免费抢58元海鲜辣椒酱呀~包邮！你也快来领~】';
            // $msg['text'] = array('content' => $str);
            // self::$_wx->sendCustomMessage($msg);
            // 上传下载相应
    ...
    public function checkEvent($event)
    {
        switch ($event) {
            ...
            case 'subscribe':
                ...
                // $newmsg = array();
                // $newmsg['msgtype'] = 'text';
                // $str = '来了？凑28个小伙伴就能得58元的海鲜辣椒酱，包邮！<a href="http://mp.weixin.qq.com/s?__biz=MzA4NDMyODYyMg==&mid=2449840648&idx=1&sn=d72af1c9046712ce33a0f4fca2defeda&scene=0#wechat_redirect">活动说明</a>！';
                // $newmsg['text'] = array('content' => $str);

                $old['openid'] = self::$_revdata['FromUserName'];
                $isold = self::$_ppvip->where($old)->find();
                if ($isold) {
                    // $newmsg['touser'] = $old['openid'];
                    // self::$_wx->sendCustomMessage($newmsg);
                    $data['subscribe'] = 1;
                    ...
                    if ($isold['pid']) {
                        ...
                        if ($fxs) {
                            ...
                            $rfxs = self::$_fx->where('id='.$isold['pid'])->setInc('total_xxsub');    //下线累计关注
                            $rfxs = self::$_fx->where('id='.$isold['pid'])->setDec('total_xxunsub');    //下线累计关注

                            // 上上线 也要累计添加
                            if ($fxs2 = self::$_fx->where('id='.$fxs['pid'])->field('id,pid')->find()) {
                                $rfxs2 = self::$_fx->where('id='.$fxs['pid'])->setInc('total_xxsub');
                                $rfxs2 = self::$_fx->where('id='.$fxs['pid'])->setDec('total_xxunsub');
                            }

                            if ($fxs3 = self::$_fx->where('id='.$fxs2['pid'])->field('id,pid')->find()) {
                                $rfxs3 = self::$_fx->where('id='.$fxs2['pid'])->setInc('total_xxsub');
                                $rfxs3 = self::$_fx->where('id='.$fxs2['pid'])->setDec('total_xxunsub');
                            }
                    ...
                    if ($user) {
                        ...
                        if ($old) {
                            $rfxs = self::$_fx->where('id='.$pid)->setInc('total_xxsub', 1);    //下线累计关注

                            // 上上线 也要累计添加
                            if ($fxs2 = self::$_fx->where('id='.$old['pid'])->field('id,pid')->find()) {
                                $rfxs2 = self::$_fx->where('id='.$old['pid'])->setInc('total_xxsub', 1);
                            }

                            if ($fxs3 = self::$_fx->where('id='.$fxs2['pid'])->field('id,pid')->find()) {
                                $rfxs3 = self::$_fx->where('id='.$fxs2['pid'])->setInc('total_xxsub', 1);
                            }
            ...
            case 'unsubscribe':
                ...
                if ($old) {
                    ...
                    if ($old['pid']) {
                        $fxs = self::$_fx->where('id='.$old['pid'])->find();
                        if ($fxs) {
                            $dlog['ppid'] = $old['ppid'];
                            $dlog['from'] = $old['id'];
                            ...
                            $dlog['ctime'] = time();
                            $rdlog = self::$_fxlog->add($dlog);
                            $rfxs = self::$_fx->where('id='.$old['pid'])->setDec('total_xxsub');
                            $rfxs = self::$_fx->where('id='.$old['pid'])->setInc('total_xxunsub');
                            // 上上线 也要累计添加
                            if ($fxs && $fxs2 = self::$_fx->where('id='.$fxs['pid'])->field('id,pid')->find()) {
                                $rfxs2 = self::$_fx->where('id='.$fxs['pid'])->setDec('total_xxsub');
                                $rfxs2 = self::$_fx->where('id='.$fxs['pid'])->setInc('total_xxunsub');
                            }

                            if ($fxs2 && $fxs3 = self::$_fx->where('id='.$fxs2['pid'])->field('id,pid')->find()) {
                                $rfxs3 = self::$_fx->where('id='.$fxs2['pid'])->setDec('total_xxsub');
                                $rfxs3 = self::$_fx->where('id='.$fxs2['pid'])->setInc('total_xxunsub');
                            }
                        }
                    } else {
                        $dlog['ppid'] = 0;
    ...
    public function toKeyUser($ruser)
    {
        ...
        switch ($type) {
            //文本
            case '1':
                $msg = html_entity_decode($ruser['summary']);
                self::$_wx->text($msg)->reply();
                break;
            //单图文
            case '2':
    ...
    // 获取头像函数
    public function getRemoteHeadImage($headimgurl)
    {
        $ch = curl_init();
    ...
    // 创建二维码
    public function createQrcode($id, $openid)
    {
        if ($id == 0 || $openid == '') {
            return false;
        }
...
//            \Util\QRcode::png($url, './QRcode/qrcode/' . $openid . '.png', 'L', 6, 2);
    ...
    // 创建二维码
    public function createEmployeeQrcode($id, $openid)
    {
        if ($id == 0 || $openid == '') {
            return false;
        }
    ...
    // 创建背景
    public function createQrcodeBg()
    {
    ...
    // 创建背景
    public function createQrcodeBgEmp()
    {
        $autoset = M('Autoset')->find();
    ...
    // 关注时返回信息
    public function subscribeReturn($msg)
    {
        // 获取wx_keyword
        $kcond = array(
            'sendatsub' => 1,
        );
        $data = M('wx_keyword')->where($kcond)->find();
        switch ($data['type']) {
            // 纯文本
            case '1':
                $toMsg = empty($data['summary']) ? $msg : $data['summary'];
                self::$_wx->text($toMsg)->reply();
                break;
            // 单图文
            case '2':
                $sendData = array(
                    array(
                        ...
                        'PicUrl' => 'http://'.$_SERVER['HTTP_HOST'].$this->getSubscribePic($data['pic']),
                        'Url' => $data['url'],
                    ),
                );
                self::$_wx->news($sendData)->reply();
                break;
            // 多图文
            case '3':
                /*$imgcond = array(
                ...
                $allitems = M('wx_keyword_img')->where($imgcond)->order('sorts desc')->select();
                file_put_contents('data.txt', json_encode($allitems));
                if (empty($allitems)) {
                    $this->defaultSubscribeReturn($msg);
                }
                ...
                self::$_wx->news($sendDatas)->reply();*/

                $pagelist = M('Wx_keyword_img')->where(array('kid' => $data['id']))->order('sorts desc')->select();
                $news = array();
                foreach ($pagelist as $k => $v) {
                    $news[$k]['Title'] = $v['name'];
                    $news[$k]['Description'] = $v['summary'];
                    $img = $this->getPic($v['pic']);
                    $news[$k]['PicUrl'] = $img['imgurl'];
                    $news[$k]['Url'] = urldecode($v['url']);
                }
                // file_put_contents('wx.txt','响应参数:'.json_encode($news).PHP_EOL.PHP_EOL.PHP_EOL,FILE_APPEND);
                self::$_wx->news($news)->reply();
                break;
            default:
                $this->defaultSubscribeReturn($msg);
    ...
    public function defaultSubscribeReturn($msg)
    {
    ...
    // 获取单张图片
    public function getSubscribePic($id)
    {
        $m = M('UploadImg');
        $temparr = split(',', $id);
