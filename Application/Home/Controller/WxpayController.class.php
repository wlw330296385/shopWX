...
//		$_SESSION['tmpwxpay']=$wOpt;
    ...
    public function doDs($order)
    {
        ...
        if ($pid) {
            ...
            $fxnameindb = M('shop_set')->field('fx1name, fx2name, fx3name')->find();
            ...
            if ($fx1) {
                ...
                if ($fxlog['fxyj']) {
                    $msg = array(
                        'touser'=>$fx1['openid'],
                        'msgtype'=>'text',
                        'text'=>array('content'=>'你的'.$fxnameindb['fx1name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！')
                    );
                ...
                if ($fx2) {
                    ...
                    if ($fxlog['fxyj']) {
                        $msg = array(
                            'touser'=>$fx2['openid'],
                            'msgtype'=>'text',
                            'text'=>array('content'=>'你的'.$fxnameindb['fx2name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！')
                        );
                        $_wx->sendCustomMessage($msg);
                    }
                    //单层逻辑
                ...
                if ($fx3) {
                    ...
                    if ($fxlog['fxyj']) {
                        $msg = array(
                            'touser'=>$fx3['openid'],
                            'msgtype'=>'text',
                            'text'=>array('content'=>'你的'.$fxnameindb['fx3name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！')
                        );
                        $_wx->sendCustomMessage($msg);
                    }
                    //单层逻辑
