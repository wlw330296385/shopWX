...
class VipController extends BaseController
{
    ...
    public function txOrder()
    {
        ...
        if (IS_POST) {
            ...
            if (FALSE !== $rvip) {
                ...
                $post['status'] = 1;
                $r = $mtx->add($post);
                // file_put_contents('txorder.txt', json_encode($post),FILE_APPEND);
                if ($r) {
                    $data_msg['pids'] = $vipid;
                    // $data_msg['title'] = "您的" . $post['txprice'] . $_SESSION['SHOP']['set']['yjname'] . "提现申请已成功提交！会在三个工作日内审核完毕并发放！";
                    $data_msg['title'] = "收到了您的提现".$post['txprice']. "元".$_SESSION['SHOP']['set']['yjname']."申请，审核过后就会给你发红包了~ 注意查收哦！";
                    ...
                    // file_put_contents('txorder.txt', json_encode($rmsg).PHP_EOL,FILE_APPEND);
                    // 发送信息===============
                    ...
                    // file_put_contents('txorder.txt', json_encode($wx).'-----3'.PHP_EOL,FILE_APPEND);
                    $msg = array();
                    $msg['touser'] = $vip['openid'];
                    $msg['msgtype'] = 'text';

                    $str = "收到了您的".$customer['value'].":".$post['txprice']."元".$_SESSION['SHOP']['set']['yjname']."，审核过后就会给你发红包了~ 注意查收哦！";
                    $msg['text'] = array('content' => $str);
                    $ree = $wx->sendCustomMessage($msg);
                    // 发送消息完成============
                    // file_put_contents('txorder.txt', json_encode($ree).PHP_EOL,FILE_APPEND);
                    $this->success('提现申请成功！');
                } else {
                    $data_msg['pids'] = $vipid;
                    // $data_msg['title'] = "您的" . $post['txprice'] . $_SESSION['SHOP']['set']['yjname'] . "提现申请已成功提交！会在三个工作日内审核完毕并发放！";
    ...
    public function addressSet()
    {
        ...
        if (IS_POST) {
            $post = I('post.');
            $post['vipid'] = $vipid;
            $post['province']=str_replace('省','',$post['province']);
            $post['city']=str_replace('市','',$post['city']);
