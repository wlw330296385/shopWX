...
class ShopController extends BaseController
{
    public function _initialize()
    {
        ...
        if (self::$SHOP['set']['tpl'] && self::$SHOP['set']['tpl'] != 'default') {
            C('DEFAULT_THEME', 'App/'.self::$SHOP['set']['tpl']);
        }
    }
...
//            }
    ...
    public function goods()
    {
        ...
        //显示评论
        $reviews = M('shop_reviews')->where(array('goodsid' => $id, 'ispassed' => 0))->select();
        $reviewsNum=0;
        if ($reviews) {
            foreach ($reviews as $key => $value) {
                if ($value['revpic']) {
                    $reviews[$key]['picurl'] = $this->getAlbum($value['revpic']);
                } else {
                    $reviews[$key]['picurl'] = null;
                }
            }
            $reviewsNum=M('shop_reviews')->where(array('goodsid' => $id, 'ispassed' => 0))->count();
           
            $this->assign('reviews', $reviews);
        }
        $this->assign('reviewsNum',$reviewsNum);
        //绑定购物车数量
        ...
        if (false !== $rod) {
            ...
            if ($pid) {
                ...
                if ($fx1) {
                    ...
                    if ($fxlog['fxyj'] < 1.0 || self::$WAP['vipset']['istxsh'] || !$this->orderRedPack($redpackDb, $txObj, $fx1, $txDb, $fxlog['fxyj'])) {
                ...
                if ($fx1['pid']) {
                    ...
                    if ($fx2) {
                        ...
                        $sendMsg = false;
                        if ($fxlog['fxyj'] < 1.0 || self::$WAP['vipset']['istxsh'] || !$this->orderRedPack($redpackDb, $txObj, $fx2, $txDb, $fxlog['fxyj'])) {
                ...
                if ($fx2['pid']) {
                    ...
                    if ($fx3) {
                        ...
                        $sendMsg = false;
                        if ($fxlog['fxyj'] < 1.0 || self::$WAP['vipset']['istxsh'] || !$this->orderRedPack($redpackDb, $txObj, $fx3, $txDb, $fxlog['fxyj'])) {
...
//		$backurl=base64_encode($orderdetail);
            ...
            case 'money':
                ...
                if ($pp >= 0) {
                    ...
                    if ($re) {
                        ...
                        if ($order['djqid']) {
                            $mcard = M('Vip_card');
                            $djq = $mcard->where('id='.$order['djqid'])->find();
                            $djq['status'] = 2;
                            $djq['usetime'] = time();
                            $rdjq = $mcard->save($djq);
                        }
                        if (false !== $rod) {
                            ...
                            if ($templateid) { // 存在才可以发送模板消息
                                ...
                                $data['data'] = array(
                                    ...
                                    'remark' => array('value' => '3-5天内发货,客服电话:17776258826'),
                                );
                                $options['appid'] = self::$_wxappid;
                                $options['appsecret'] = self::$_wxappsecret;
                                $wx = new \Util\Wx\Wechat($options);
                            ...
                            // 插入订单支付成功模板消息结束=================
                            // 发送商家模板消息
                            $wx->sendMobanMsmToShop($order['oid'],1);
                            //打印机
                            $Tx = new \Util\Tx();
                            $Tx->sendPrintPP($order);
                            //首次支付成功自动变为花蜜
                            if ($vip && !$vip['isfx']) {
    ...
    public function doDs($order)
    {
        ...
        if ($pid) {
            ...
            if ($fx1) {
                ...
                //你的X辣好友，买了X瓶辣椒酱，确认收货后你就有xx元红包到账哦！加油！
                if ($fxlog['fxyj']) {
                    $msg = array(
                        'touser' => $fx1['openid'],
                        'msgtype' => 'text',
                        'text' => array('content' => '你的'.self::$WAP['shopset']['fx1name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！'),
                    );
                    $_wx->sendCustomMessage($msg);
                }
                ...
                if ($fx2) {
                    ...
                    if ($fxlog['fxyj']) {
                        $msg = array(
                            'touser' => $fx2['openid'],
                            'msgtype' => 'text',
                            'text' => array('content' => '你的'.self::$WAP['shopset']['fx2name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！'),
                        );
                        $_wx->sendCustomMessage($msg);
                    }
                    //单层逻辑
                ...
                if ($fx3) {
                    ...
                    if ($fxlog['fxyj']) {
                        $msg = array(
                            'touser' => $fx3['openid'],
                            'msgtype' => 'text',
                            'text' => array('content' => '你的'.self::$WAP['shopset']['fx3name'].'['.$vip['nickname'].']，买了'.$order['totalnum'].'瓶辣椒酱，确认收货后你就有'.$fxlog['fxyj'].'元红包到账哦！加油！'),
                        );
                        $_wx->sendCustomMessage($msg);
                    }
                    //单层逻辑
    ...
    public function reviews()
    {
        ...
        if (IS_POST) {
            ...
            $shopReviews = M('shop_reviews');
            $data = $_POST;
            $setField = array(
                'isreviewed' => 1,
                'vipnickname' => $_SESSION['WAP']['vip']['nickname'],
                'headimgurl' => $_SESSION['WAP']['vip']['headimgurl'],
                'vipopenid' => $_SESSION['WAP']['vip']['openid'],
                'goodsid' => $data['goodsid'],
                'goodsmark' => $data['goodsmark'],
                'servermark' => $data['servermark'],
                'deliverymark' => $data['deliverymark'],
                'revcontent' => $data['revcontent'],
            ...
            $msg['status'] = 0;
            //查询数据库是否已经评论
            $re = $shopReviews->where($orderCond)->find();
            //修改订单表的items
            $order = M('shop_order')->where('id='.$orderCond['orderid'])->find();
            $items = unserialize($order['items']);
            foreach ($items as $key => $value) {
                if ($value['goodsid'] == $orderCond['goodsid']) {
                    $items[$key]['isreviewed'] = 1;
                }
            }
            $items = serialize($items);
            M('shop_order')->where('id='.$orderCond['orderid'])->setField(array('items' => $items));
            if ($re) {
                $shopReviews->save($setField);
                $msg['status'] = 1;
                die(json_encode($msg));
            } else {
                $shopReviews->add($setField);
                $msg['status'] = 1;
                die(json_encode($msg));
            }
        } else {
    ...
    private function orderRedPack(&$redpackDb, &$txObj, &$vip, &$txDb, $amount)
    {
        ...
        $redpack = array(
            ...
            'mch_billno' => self::$SET['wxmchid'].date('Ymd').$extraStr,
            'mch_id' => self::$SET['wxmchid'],
            'wxappid' => self::$SET['wxappid'],
            'send_name' => '小海家辣椒酱',
        ...
        // $txObj = new \Util\Tx();
        $sign = $txObj->getSign($redpack, self::$SET['wxmchkey']);
        $redpack['sign'] = $sign;
