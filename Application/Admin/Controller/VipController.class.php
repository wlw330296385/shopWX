...
class VipController extends BaseController
{
    ...
    public function messageSet()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
                ...
                if ($data['pids']) {
                    $options = array(
                        'token'=>self::$SYS['set']['wxtoken'],
                        'appid'=>self::$SYS['set']['wxappid'],
                        'appsecret'=>self::$SYS['set']['wxappsecret']
                    );
                    ...
                    $openid = M('vip')->where($cond)->getField('openid');
                    $msgData = array(
                        'touser'=>$openid,
                        'msgtype'=>'text',
                        'text'=>array('content'=>$data['content'])
                    );
                    $wx->sendCustomMessage($msgData);
                }
        ...
        if ($re) {
            ...
            if (!empty($_POST['sendMsg'])) {
                ...
                $msgData = array(
                    'touser'=>$openid,
                    'msgtype'=>'text',
                    'text'=>array('content'=>$_POST['sendMsg'])
                );
                $wx->sendCustomMessage($msgData);
            }
    ...
    public function sendredpack()
    {
        ...
        $txCond = array(
            'id'=>$txId,
            'status'=>1,
            'txtime'=>array('exp', 'is null')
        );
        $txDB = M('vip_tx');
        if (!($vipid = $txDB->where($txCond)->field('vipid, txprice')->find())) {
            $info = array(
                'status'=>0,
                'msg'=>'找不到'
            );
        ...
        if (!($openid = M('vip')->where($vipCond)->getField('openid'))) {
            $info = array(
                'status'=>0,
                'msg'=>'找不到'
            );
        ...
        $redpack = array(
            ...
            "send_name"=>'小海家的辣椒酱',
            "re_openid"=>$openid,
            "total_amount"=>intval(floatval($vipid['txprice']) * 100),
            "total_num"=>1,
            "wishing"=>'恭喜您，提现成功',
        ...
        if ($retArr['return_code'] == 'SUCCESS') {
            ...
            // 成功，设置提现状态
            if ($txDB->where($txCond)->setField($setField) !== false) {
                $info = array(
                    'status'=>1,
                    'msg'=>'提现成功'
                );
